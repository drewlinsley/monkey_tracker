...
def create_depth_graph(all_xy, theta, hw, resize):
    ...
    for i, th in tqdm(enumerate(theta), total=len(theta)):
        """Daniel: Compare im1 - depth image and im1 - im2."""
        ...
        if i == 0:
            ...
            dms = tf.concat(axis=2, values=[dms, sel_slice])
    return dms, depth_image


def pad_image(depth_image, offset, hw):
    """Daniel: Check that this works appropriately."""
    offset *= -1
    if offset[0] < 0 and offset[1] < 0:
        # +, +
        crop_im = depth_image[:offset[0], :offset[1]]
        pad_im = tf.concat(
            axis=1, values=[tf.zeros([hw[0],
                np.abs(offset[1])]), tf.concat(
                axis=0, values=[tf.zeros(
        ...
        crop_im = depth_image[:offset[0], offset[1]:]
        pad_im = tf.concat(
            axis=1, values=[tf.concat(axis=0,
                values=[tf.zeros([
        ...
        pad_im = tf.concat(
            axis=1, values=[tf.zeros([hw[0],
                np.abs(offset[1])]), tf.concat(axis=0,
                values=[crop_im, tf.zeros(
...
def get_label(raw_labels, background_constant, label_values, resize=None):
    ...
    label_matrix = np.asarray(
        [label_values[k] for k in sorted(label_values.keys())])
    assert label_im.shape[2] == label_matrix.shape[1]
