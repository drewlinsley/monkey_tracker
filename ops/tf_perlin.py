...
def get_noise(h, w):
    ...
    lin_h = np.linspace(0, np.random.randint(4) + 1, h, endpoint=False)
    lin_w = np.linspace(0, np.random.randint(4) + 1, w, endpoint=False)
    ...
    v = fade(yf)
    # Perlin noise
    return perlin(h, w, x, y, xi, yi, xf, yf, u, v, vectors)
