...
def encode_example(im, label, im_label=None):
...
def create_joint_tf_records(
        depth_files, label_files, tf_file, config, sample=True):
    ...
    with tf.python_io.TFRecordWriter(tf_file) as tfrecord_writer:
        for i, (depth, label) in tqdm(
            ...
            example = encode_example(depth_image, label_vector, im_label)
