...
def pad_image(depth_image, offset, hw):
    ...
    if zero_lt is not None and one_lt is not None:
        ...
        pad_im = tf.concat(axis=1,
            values=[tf.zeros([hw[0],
        ...
        pad_im = tf.concat(axis=1,
            values=[tf.zeros([hw[0],
                np.abs(offset[1])]), tf.concat(axis=0,
                values=[crop_im, tf.zeros([offset[0], hw[1] - np.abs(offset[1])])])])  # c_zeros + image + r_zeros
    elif zero_gt is not None and one_gt is not None:
        # -, -
        crop_im = depth_image[offset[0]:, offset[1]:]
        pad_im = tf.concat(axis=1,
            values=[tf.concat(axis=0,
                values=[crop_im, tf.zeros([offset[0], hw[1] - np.abs(offset[1])])]), tf.zeros(
