...
def save_mosaic(
    ...
    for idx, (im, yhat) in enumerate(zip(ims, yhats)):
        ...
        if conv_xy_to_hw is not None:
            yhat = yhat.reshape(-1, num_dims)
            yhat[:, 0] = yhat[:, 0] * conv_xy_to_hw[0]
            yhat[:, 1] = yhat[:, 1] * conv_xy_to_hw[1]
            yhat = yhat.reshape(-1)
            ytrue = ytrue.reshape(-1, num_dims)
        ...
        lab_legend_artists = plot_coordinates(
            ax1, ytrue, colors, num_dims, marker='.', markersize=1.5)
        leg_title = 'Predicted | True'
        est_legend_artists = plot_coordinates(
            ax1, yhat, colors, num_dims,
            linestyle='none',
            markeredgewidth=.5,
            marker='o',
            mfc='none',
...
def plot_coordinates(ax, vector, colors, num_dims, **kwargs):
    if ax is None:
        fig, ax = plt.subplots()
    xy = xyz_vector_to_xy(vector, num_dims)
    if colors is None:
        colors = ['r'] * xy.shape[0]
