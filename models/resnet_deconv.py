...
class model_struct:
        ...
        layer_structure = [
            {
                'layers': ['conv', 'deconv'],
                'weights': [256, 256],
                'names': ['up_conv_1', 'up_deconv_1'],
                'filter_size': [3, 3]
            },
            {
                'layers': ['conv', 'deconv'],
                'weights': [64, 64],
                'names': ['up_conv_2', 'up_deconv_2'],
                'filter_size': [3, 3]
            },
            {
                'layers': ['conv', 'deconv'],
                'weights': [1, 1],
                'names': ['up_conv_3', 'up_deconv_3'],
                'filter_size': [3, 3]
            }
            ]
    ...
    def deconv_layer(
            ...
            stride=[1, 1, 1, 1],
            padding='SAME'):
        conv_name = name.split('deconv')[0]
        argmax_name = conv_name + '_argmax'
        with tf.variable_scope(name):
            filt = [filter_size, filter_size, in_channels, out_channels]
            act = self.deconv(
                bottom,
                filt,
                in_channels)
    ...
    def deconv(
            ...
            stride=[1, 1, 1, 1],
            padding='SAME'):
        W = self.weight_variable(filt)
        b = self.bias_variable([in_channels])

        bottom_shape = tf.shape(bottom)
        out_shape = tf.pack(
            [bottom_shape[0], bottom_shape[1], bottom_shape[2], filt[2]])
