...
def main(labelDir):
	imDir = os.path.join(labelDir, 'layer2')
	coordDir = os.path.join(labelDir, 'joint_coords')
	pixelDir = os.path.join(labelDir, 'pixel_joint_coords')
	occlusionDir = os.path.join(labelDir, 'occlusions')
	os.makedir(pixelDir) 
	os.makedir(occlusionDir)
	...
	for filename in tqdm(os.listdir(coordDir)):
		coords = np.load(os.path.join(coordDir, filename))
		image = misc.imread(os.path.join(imDir, filename[:-4]+'.png'))
		pixel_coords = (coords[:, :2]*np.asarray([-620.0, -620.0])/coords[:, 2].reshape([23, 1]) + 
			np.asarray([320.0, 240.0])).astype(np.int32)
		labels = image[[pixel_coords[:, 1], pixel_coords[:,0]]]
		...
		np.save(os.path.join(pixelDir, filename), pixel_coords)
		np.save(os.path.join(occlusionDir, filename), visible)
